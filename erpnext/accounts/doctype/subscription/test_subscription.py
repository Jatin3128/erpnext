...
class TestSubscription(ERPNextTestSuite):
	...
	def test_subscription_cancels_at_period_end_without_end_date(self):
		...
		# next_billing_period_end forward to the next period before this check runs,
		# so with no end_date to fall back on, cancel_at_period_end must compare
		...
		self.assertEqual(len(subscription.invoices), 0)
		period_end = subscription.next_billing_period_end
